...
function assembleGridAST(fileData, gridColumns = [1, 1, 2, 3], imageWidth, imageHeight, gridIncludeBody) {
  ...
  const grid = {
    type: 'grid',
    columns: columns,
    children: fileData.map(({ title, author, description, thumbnail, filename, bodyNodes }) => {
      const cardChildren = [
...
const documentListingDirective = {
  ...
  run(data) {
    ...
    const result = (type === 'grid')
      ? assemblyFunction(limitedFileData, gridCols, imageWidth, imageHeight, gridIncludeBody)
      : (type === 'summary')
...
export const plugin = {
  name: 'document-listing',
  directives: [documentListingDirective]
};
